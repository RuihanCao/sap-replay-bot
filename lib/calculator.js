...
async function buildWinPercentReportHeadless(battleJsonList) {
  const results = [];
...
function getDebugBattle(battleJson) {
  const calculatorState = parseReplayForCalculator(battleJson);
  const config = {
    ...calculatorState,
    playerToyLevel: Number(calculatorState.playerToyLevel) || 1,
    opponentToyLevel: Number(calculatorState.opponentToyLevel) || 1,
...
function getDebugSequence(battleJsonList, targetIndex) {
  const results = [];
  let targetLogs = [];
